...
my %keyboards = (
	pc => {
		rows => [
			...
			[qw(TAB Q W E R T Y U I O P LEFT_BRACKET RIGHT_BRACKET ENTER)],
			[qw(CAPS_LOCK A S D F G H J K L SEMICOLON APOSTROPHE BACKSLASH)],
			[qw(SHIFT_LEFT PLUS Z X C V B N M COMMA PERIOD SLASH SHIFT_RIGHT)],
